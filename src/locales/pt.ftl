...
### MYINFO ###
###############
myinfo-title = Suas informacoes
myinfo-user = Usuario: { $name }
myinfo-id = ID: { $id }
myinfo-total = Total de { $genero }: { $total }
myinfo-harem = Harem: { $userTotal } de { $dbTotal } ({ $percent }%)
...
gift_reply_instruction= use em reply a mensagem de confirmação para presentear o personagem
...
success_dominar_rarity = ✨ <b>Raridade:</b> { $rarity }
...
harem_inline_caption_rarity = {$rarity_emoji} Raridade:  { $rarity_name } { $rarity_emoji }
